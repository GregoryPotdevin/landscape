...
export class Flag extends React.Component {
  render() {
    const { label, showLabel, country } = this.props
    const code = countryCodeFor(label || country || this.props.code)
    if (showLabel){
      return (
        <div>
          <div className={"flag flag-" + code} style={{margin: 'auto'}} />
          <div>{label}</div>
        </div>
      )
    } else {
      return <div className={"flag flag-" + code} style={{margin: 'auto'}} /> 
    }
  }
}
